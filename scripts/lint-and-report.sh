...
run_linter() {
  ...
  case "$lang" in
    python)
      if check_tool uv && [[ -f "$dir/pyproject.toml" ]]; then
        echo "--- ruff ---"
        (cd "$dir" && uv run ruff check . 2>&1) || true
        echo "--- mypy ---"
        (cd "$dir" && uv run mypy src/ tests/ 2>&1) || true
      elif [[ -f "$dir/.venv/bin/ruff" ]]; then
        echo "--- ruff ---"
        (cd "$dir" && "$dir/.venv/bin/ruff" check . 2>&1) || true
        if [[ -f "$dir/.venv/bin/mypy" ]]; then
          echo "--- mypy ---"
          (cd "$dir" && "$dir/.venv/bin/mypy" src/ tests/ 2>&1) || true
        fi
      else
        if check_tool ruff; then
          ...
          echo "SKIP: ruff not found (pip install ruff or uv add ruff)"
        fi
        if check_tool mypy; then
          echo "--- mypy ---"
          (cd "$dir" && mypy src/ tests/ 2>&1) || true
        fi
      fi
      ;;
